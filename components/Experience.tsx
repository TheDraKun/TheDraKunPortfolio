          ...
          <div className="space-y-16">
            {EXPERIENCE.map((exp, idx) => (
              <div key={idx} className="relative pl-12">
                {/* Marker - Removed scaling animations */}
                <div className="absolute left-4 top-11 -translate-x-1/2">
                  <div className="h-3 w-3 rounded-full bg-[#E80368] shadow-[0_0_15px_rgba(232,3,104,0.7)]" />
                </div>
                
                <div className="bg-[#111827]/40 border border-[#1e293b] p-8 md:p-10 rounded-[2rem] shadow-xl">
                  ...
                  <ul className="space-y-4">
                    {exp.achievements.map((ach, i) => {
                      ...
                      return (
                        <li key={i} className={`flex gap-4 ${isProject ? 'mt-8 mb-2' : ''}`}>
                          {!isProject && (
                            <span className="text-[#E80368] font-black mt-1">▶</span>
                          )}
                          <span className={`text-sm md:text-base leading-relaxed ${isProject ? 'text-white font-bold' : 'text-white/60'}`}>
